...
`;
  ...
  try {
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues`, {
      ...
      body: JSON.stringify({
        title: `🧹 Dependency Cleanup: ${unusedFiles.length} unused files found`,
        body,
...
export const checkHealthThreshold = (threshold: number = 70): boolean => {
  ...
  if (health.healthScore < threshold) {
    const message = `❌ Project health (${health.healthScore}%) is below threshold (${threshold}%). Found ${health.unusedFiles} unused files.`;
  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        attachments: [{
          color,
          title: '🧠 WWCD Dependency Health Report',
          fields: [
            {
              title: 'Health Score',
